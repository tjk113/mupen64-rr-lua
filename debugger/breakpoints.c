...
// TODO: check_breakpoints() being a critical bottleneck when run = 1,
// it should be optimised to maximum.
// First solution: sort bp_addresses[] to shorten while loop.
// Second solution: use a hash function, but efficiency has
// to be verified (e.g. in "2 breakpoints defined" case).
...
//]=-=-=-=-=-=-=[ Signal Return Functions (CallBack) ]=-=-=-=-=-=-=[
...
static void on_add()
{
	...
	//FIX NEEDED: colors are not updated + everything
}
