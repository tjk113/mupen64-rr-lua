...
void SaveRomBrowserCache() {
	...
	if (cacheFile) {
		...
		gzwrite(cacheFile, ItemList.List, sizeof(t_rom_header) * ItemList.ListCount);
		gzclose(cacheFile);
	}
}
...
BOOL LoadRomBrowserCache() {
	...
	if (cacheFile) {
		...
		ItemList.List = (t_rom_header*)malloc(sizeof(t_rom_header) * ItemList.ListCount);
		ItemList.ListAlloc = ItemList.ListCount;
		gzread(cacheFile, ItemList.List, sizeof(t_rom_header) * ItemList.ListCount);
		gzclose(cacheFile);
...
void CountryCodeToCountryName(int countrycode, char* countryname) {  //Thuis function is from 1964 sources,Thanks to Schibo for allowing me to use it
...
void AddRomToList(char* RomLocation) {
	...
	if (ItemList.ListAlloc == 0) {
		ItemList.List = (t_rom_header*)malloc(100 * sizeof(t_rom_header));
		ItemList.ListAlloc = 100;
	} else if (ItemList.ListAlloc == ItemList.ListCount) {
		ItemList.ListAlloc += 100;
		ItemList.List = (t_rom_header*)realloc(ItemList.List, ItemList.ListAlloc * sizeof(t_rom_header));
	...
	memset(pRomInfo, 0, sizeof(t_rom_header));
	memset(&lvItem, 0, sizeof(lvItem));
...
void RomList_GetDispInfo(LPNMHDR pnmh) {
	...
	t_rom_header* pRomInfo = &ItemList.List[lpdi->item.lParam];
...
int CALLBACK RomList_CompareItems(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort) {
	...
	t_rom_header* pRomInfo1 = &ItemList.List[SortAscending ? lParam1 : lParam2];
	t_rom_header* pRomInfo2 = &ItemList.List[SortAscending ? lParam2 : lParam1];
