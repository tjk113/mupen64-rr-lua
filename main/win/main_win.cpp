...
int emu_paused;
int recording;
HWND hTool, mainHWND, hStatus, hRomList, hStatusProgress, game_hwnd;
...
//			count = 100;
	...
	if ( EmuThreadHandle != NULL || ExitCode != 0) {  
			...
			char str [256];
			sprintf(str, "The emu thread crashed");
			ShowError(str);
...
void resetEmu() 
{
	...
	if (emu_launched) {
		...
		// we can't do this later in the closeRom thread, so we gotta do it now on ui thread
		DestroyGameWindow();
...
static DWORD WINAPI ThreadFunc(LPVOID lpParam)
{
	...
	printf("Core type is %d" , Config.core_type);
	...
	printf("Doing commandline work if needed...\n");
	CreateThread(NULL, 0, StartMoviesThread, NULL, 0, NULL);
	StartLuaScripts();
	StartSavestate();
	AtResetCallback();

	printf("Entering game code and busy waiting loop...\n");
